...
export default async function handler(req, res) {
  ...
  } else if (req.method === 'DELETE' && req.query.cleanup === 'drawn') {
    return handleCleanupDrawnDuels(req, res);
  } else {
...
async function handleCleanupDrawnDuels(req, res) {
  let client;
  try {
    client = await pool.connect();
    ...
    const findQuery = `
      ...
      WHERE (creator.email = 'pop@proofofputt.com' OR invited.email = 'pop@proofofputt.com')
        ...
        AND d.duel_creator_score = 0 
        AND d.duel_invited_player_score = 0
      ORDER BY d.created_at DESC;
    `;
