...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!decoded || !decoded.playerId) {
      logger.warn('Invalid JWT token - missing playerId');
      logApiResponse('/api/subscriptions/bundles/purchase', 'POST', 401, {
        requestId,
        reason: 'invalid_jwt'
      });
    ...
    if (userResult.rows.length === 0) {
      logger.warn('User not found', { playerId: decoded.playerId });
      logApiResponse('/api/subscriptions/bundles/purchase', 'POST', 401, {
        requestId,
    ...
    const orderPayload = {
      ...
      metadata: {
        ...
        playerId: user.player_id.toString(),  // Explicit player_id reference for consistency
